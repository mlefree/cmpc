...
jQuery.each( ("blur focus focusin focusout load resize scroll unload click dblclick " +
	"mousedown mouseup mousemove mousewheel mouseover mouseout mouseenter mouseleave " +
...
function createHttpBackend($browser, XHR, $browserDefer, callbacks, rawDocument, locationProtocol) {
  ...
  return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
    ...
    function completeRequest(callback, status, response, headersString) {
      // URL_MATCH is defined in src/services/location.js
 ...
 * @param {number} which Optional button number for mouse/touch events.
 * @param {number} x Optional x-coordinate for mouse/touch events.
 * @param {number} y Optional y-coordinate for mouse/touch events.
 * @param {number} wheelDeltaX Optional x-delta for wheel events.
 * @param {number} wheelDeltaY Optional y-delta for wheel events.
 */
function browserTrigger(element, type, keys, which, x, y, wheelDeltaX, wheelDeltaY) {
  ...
  if (msie < 9) {
    ...
    return ret;
      ...
      var evnt;
    if (type == 'sensewheel') {
        evnt = document.createEvent('WheelEvent');
    } else {
        evnt = document.createEvent('MouseEvent');
    }
    ...
    evnt.preventDefault = function() {
      ...
      y = y || 0;
      if (type == 'sensedown') {
          console.log("browserTrigger sensedown");
          evnt.initMouseEvent('mousedown', true, true, window, 0, x, y, x, y, pressed('ctrl'), pressed('alt'),
              pressed('shift'), pressed('meta'), which, element);
      } else if (type == 'senseup') {
          console.log("browserTrigger senseup");
          evnt.initMouseEvent('mouseup', true, true, window, 0, x, y, x, y, pressed('ctrl'), pressed('alt'),
              pressed('shift'), pressed('meta'), which, element);
      } else if (type == 'sensemove') {
          console.log("browserTrigger sensemove");
          // DOM.2
          // initMouseEvent(eventType, canBubble, cancelable, viewArg, detailArg,
          //                screenXArg, screenYArg, clientXArg, clientYArg,
          //                ctrlKeyArg, altKeyArg, shiftKeyArg, metaKeyArg,
          //                buttonArg, relatedTargetArg)
          evnt.initMouseEvent('mousemove', true, true, window, 0, x, y, x, y, pressed('ctrl'), pressed('alt'),
              pressed('shift'), pressed('meta'), which, element);
      } else if (type == 'sensewheel') {
          console.log("browserTrigger sensewheel");
          // DOM.2
          // initUIEvent(eventType, canBubble, cancelable, viewArg, detailArg);
          // for mouse wheel : detailArg == delta (multiple of 3) < 0 if UP, > 0 if DOWN
          // for WheelEvent in Explorer9 : wheelDelta (multiple of 120) > 0 if UP, < 0 if DOWN
          //evnt.initUIEvent('scroll', true, true, window, 0);
          var delta = Math.floor((wheelDeltaY != 0) ? -wheelDeltaY : -wheelDeltaX)/36;
          evnt.initUIEvent('mousewheel', true, true, window, delta);
      } else {
...
(function(fn){
  ...
  fn.trigger = function(type, data) {
    if (/(click|change|keydown|blur|input|sensedown|senseup|sensemove|sensewheel)/.test(type)) {
      var processDefaults = [];
      var keys = [];
        if (data && data['metaKey']) keys.push('meta');
        if (data && data['shiftKey']) keys.push('shift');
        if (data && data['ctrlKey']) keys.push('ctrl');
        if (data && data['altKey']) keys.push('alt');
      var x = 0, y = 0;
        if (data && data['clientX']) x = data['clientX'];
        if (data && data['clientY']) y = data['clientY'];
      var which = 0;
        if (data && data['which']) which = data['which'];
        var wheelDeltaX = 0, wheelDeltaY = 0;
        if (data && data['wheelDelta']) wheelDeltaY = data['wheelDelta'];
        if (data && data['wheelDeltaX']) wheelDeltaX = data['wheelDeltaX'];
          if (data && data['wheelDeltaY']) wheelDeltaY = data['wheelDeltaY'];
      this.each(function(index, node) {
        processDefaults.push(browserTrigger(node, type, keys, which, x, y, wheelDeltaX, wheelDeltaY));
      });
...
angular.scenario.dsl('element', function() {
  ...
  chain.dblclick = function() {
    ...
    // CCN patch : add scenario mouse events triggering to trigger a4p.sense library
    chain.sensedown = function (button, x, y) {
        button = button || 0;
        // Add which for click: 1 === left; 2 === middle; 3 === right
        // Note: button is not normalized, so don't use it
        var which = (button & 1 ? 1 : (button & 2 ? 3 : (button & 4 ? 2 : 0)));
        x = x || 0;
        y = y || 0;
        return this.addFutureAction("element '" + this.label + "' sensedown " + button + " (" + x + "," + y + ")",
            function ($window, $document, done) {
                var elements = $document.elements();
                elements.trigger('sensedown', {'button':button, 'which':which, 'clientX':x, 'clientY':y});
                done();
            });
    };
    chain.senseup = function (button, x, y) {
        button = button || 0;
        // Add which for click: 1 === left; 2 === middle; 3 === right
        // Note: button is not normalized, so don't use it
        var which = (button & 1 ? 1 : (button & 2 ? 3 : (button & 4 ? 2 : 0)));
        x = x || 0;
        y = y || 0;
        return this.addFutureAction("element '" + this.label + "' senseup " + button + " (" + x + "," + y + ")",
            function ($window, $document, done) {
                var elements = $document.elements();
                elements.trigger('senseup', {'button':button, 'which': which, 'clientX': x, 'clientY': y});
                done();
            });
    };
    chain.sensemove = function (button, x, y) {
        button = button || 0;
        // Add which for click: 1 === left; 2 === middle; 3 === right
        // Note: button is not normalized, so don't use it
        var which = (button & 1 ? 1 : (button & 2 ? 3 : (button & 4 ? 2 : 0)));
        x = x || 0;
        y = y || 0;
        return this.addFutureAction("element '" + this.label + "' sensemove " + button + " (" + x + "," + y + ")",
            function ($window, $document, done) {
                var elements = $document.elements();
                elements.trigger('sensemove', {'button':button, 'which': which, 'clientX': x, 'clientY': y});
                done();
            });
    };
    chain.sensewheel = function (button, x, y, deltaX, deltaY) {
        button = button || 0;
        // Add which for click: 1 === left; 2 === middle; 3 === right
        // Note: button is not normalized, so don't use it
        var which = (button & 1 ? 1 : (button & 2 ? 3 : (button & 4 ? 2 : 0)));
        ...
        deltaX = deltaX || 0;
        deltaY = deltaY || 10;
        return this.addFutureAction("element '" + this.label + "' sensewheel " + button + " (" + x + "," + y + ")"
            + " delta(" + deltaX + "," + deltaY + ")",
            function ($window, $document, done) {
                var elements = $document.elements();
                elements.trigger('sensewheel', {'button':button, 'which': which, 'clientX': x, 'clientY': y,
                    'wheelDeltaX': -deltaX*12, 'wheelDeltaY': -deltaY*12});
                done();
            });
    };
